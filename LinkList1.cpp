...
const成员函数  《面向对象程序设计 ---c++语言描述》
	如果一个成员函数不需要直接或间接（通过调用其他的成员函数来改变其对象状态）地来改变该函数
	所属的任何数据成员，那么最好将这个成员函数标记为const
	带有const的成员函数不得更改任何数据成员，因此我们称这种类型的函数为只读函数。
	将成员函数标记const可以预防对该函数所属对象的数据成员的误写，同时有些编译器还可对这种情况
	进行优化。
*****************************************************/
